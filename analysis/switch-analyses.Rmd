...
#ggsave('plots/fig1-switch-example.pdf', clusterplot)  
```
...
knitr::kable(individual_model_results_table)
```
...
# ggsave('plots/fig2-multimodal_heatmap_participant.pdf', 
...
knitr::kable(multiple_regression_all)
```
...
#ggsave('plots/fig3-lexical_plot.pdf', units = "in",width = 12, height = 4)
```
...
knitr::kable(rater_model_results_table)
```
...
```{r}
idio_data %>% filter(subject == 50003) %>%
...
#ggsave('plots/fig4-idio_lexical_plot.pdf', units = "in",width = 15, height = 4)
...
LEA_idio %>%
  ...
  labs(title = "Distribution of Individual-Level Idiosyncratic\nScores for Different Domains",
...
ggsave('plots/fig5-idio_distribution.pdf', units = "in", width = 5, height = 5)
...
#ggsave('plots/fig6-idio_permutations.pdf', units = "in")
...
ggsave('plots/fig7-idio_clusters_plot.pdf', idio_clusters_plot, height = 5, width = 5)  
```
