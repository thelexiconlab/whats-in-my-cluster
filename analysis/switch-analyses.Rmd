...
# multimodal delta alpha = 1 so not running
# m6 = glmer(data = a, participant_designated_switch ~ norms_associative + norms_categorical+ simdrop + delta_rise_0_75_fall_0_75 + 
#   multimodaldelta_alpha_0_8_rise_1_0_fall_0_25 + (1|subject), family = binomial )
...
# sent to richard lim on May 30 2024 for idiosyncratic analyses on zemla data
# x = nonfirst_trials %>% filter(domain == "animals")  %>% rename(current_word = cluster) %>%
#   mutate(prev_word = sub(",.*", "", current_pair)) %>%
#   select(ID, subject, prev_word, current_word, response_number, response, rt)
# write.csv(x, file = "pairwise_ratings.csv", row.names = FALSE)
```
...
summary(idio_switch_model_participant)
